...
- [x] In the `new_resizer` body, define an inner function that takes two optional integer inputs `min_width` and `min_height` with default values of `0`, and in its body:
  ...
  - [x] Define an innermost function that takes two integer inputs `width` and `height`, and in its body:
    - [ ] Use the built-in min and max functions to reduce width if it's above max_width or increase it if it's below min_width. See the tip below for info about how this works.
